...
//------------------------------------------------------------------------------
inline real32_t round(const real32_t &value, const size_t &decimal,
                      RoundingMethod_t method = NEAREST) {
  real32_t scale = std::pow(10.0f, static_cast<real32_t>(decimal));
  switch (method) {
  case NEAREST:
...
//------------------------------------------------------------------------------
inline real64_t round(const real64_t &value, const size_t &decimal,
                      RoundingMethod_t method = NEAREST) {
  real64_t scale = std::pow(10.0, static_cast<real64_t>(decimal));
  switch (method) {
  case NEAREST:
    return std::round(value * scale) / scale;
  case CEIL:
