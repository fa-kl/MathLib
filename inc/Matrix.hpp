...
public:
  ...
  data_t& operator()(const index_t i)
  {
    ...
    size_t idx = i > 0 ? static_cast<size_t>(i - 1) : static_cast<size_t>(len + i);
    if (idx >= length()) {
      throw IndexOutOfRangeError();
    }
    return m_data[idx];
  ...
  data_t& operator()(const index_t row, const index_t col)
  {
    if (row == 0 || col == 0) {
      throw IndexOutOfRangeError("0 is an invalid index for 1-based access methods");
    }
    size_t r = row > 0 ? static_cast<size_t>(row - 1) : static_cast<size_t>(static_cast<index_t>(m_size.rows) + row);
    size_t c = col > 0 ? static_cast<size_t>(col - 1) : static_cast<size_t>(static_cast<index_t>(m_size.cols) + col);
...
Matrix<data_t> hcat(const Matrix<data_t>& lhs, const Matrix<data_t>& rhs)
{
  ...
  for (size_t col = 0; col < lhs.cols(); ++col) {
    for (size_t i = 0; i < lhs.rows(); ++i) {
      result[col * result.rows() + i] = lhs[col * lhs.rows() + i];
    }
  }
  for (size_t col = 0; col < rhs.cols(); ++col) {
    for (size_t i = 0; i < rhs.rows(); ++i) {
      result[(lhs.cols() + col) * result.rows() + i] = rhs[col * rhs.rows() + i];
    }
  }
  return result;
}
...
Matrix<data_t> vcat(const Matrix<data_t>& lhs, const Matrix<data_t>& rhs)
{
  ...
  for (size_t col = 0; col < lhs.cols(); ++col) {
    ...
    for (size_t i = 0; i < rhs.rows(); ++i) {
      result[col * result.rows() + lhs.rows() + i] = rhs[col * rhs.rows() + i];
    }
  }
  return result;
}
...
/*
  TODO: vec*matrix, matrix*vec, matrix*matrix
        svd,
*/
...
template <typename data_t>
QRDecomposition<data_t> qr(const Matrix<data_t>& A, bool checkRank = true)
{
  size_t m = A.rows();
  size_t n = A.cols();
  ...
  for (index_t col = 1; col <= static_cast<index_t>(n); ++col) {
    ...
    // R[col,col] = norm
    R(col, col) = static_cast<data_t>(norm);
